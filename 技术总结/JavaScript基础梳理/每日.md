...
BOM: Browser Object Model, 核心对象是window
...
+ 获取节点
  ...
  ```js
  // querySelector(任意css选择器)：获取单个DOM元素（从上至下优先匹配第一个）
  ...
  // querySelectorAll(任意css选择器)：获取所有匹配的DOM元素列表（使用方法与querySelector一致）

  // getElementById('id值')：指定的id的对象引用

  // getElementByClassName('class属性值')：所有匹配的class的元素集合
